...
import { applyBattleAction, chooseAiAction, createBattle, nextActor, validActions, type BattleAction, type BattleState } from "../systems/battle-engine.ts";

export { content };
export const SPRITE_SPECIES_ALIASES: Readonly<Record<string, string>> = { bogcrumbler: "bogrumbler", knubback: "knobback" };
export interface TrainingFrame { action: BattleAction; before: BattleState; after: BattleState }
...
interface TrainingSave { version: 1; rules: string; playerId: string; enemyId: string; seed: number; actions: BattleAction[] }

/** Offline exhibition: uses production combat, but never grants campaign rewards. */
export class TrainingBattle {
  state: BattleState;
  private rng: SeededRandom;
  private history: BattleAction[] = [];
  private setup: { playerId: string; enemyId: string; seed: number };
  constructor(playerId: string, enemyId: string, seed = 42) {
  ...
  step(action: BattleAction): TrainingFrame {
  ...
  save(): string {
    const save: TrainingSave = { version: 1, rules: RULES, ...this.setup, actions: this.history };
    return JSON.stringify(save);
  }
  static restore(serialized: string): TrainingBattle {
