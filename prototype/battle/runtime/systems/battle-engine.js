...
import { calculateDamage, createCombatant, typeMultiplier } from "./combat.js";
import { evaluateTeamSynergies, passiveForMonster } from "./team-effects.js";
...
function passiveForUnit(unit                             , content                                 )                    {
...
function isControlSkill(skill                 )          {
  return Boolean(skill.statusId) || skill.power === 0;
}

function actionEnergyCost(unit            , skill                 , content                                 )         {
...
function refreshTeamStats(units              , content               )                                                  {
...
export function createBattle(
  ...
  const makeUnits = (monsters                              , side            )               => monsters.slice(0, 5).map((monster, index) => {
  ...
  const synergyEvents                = Object.entries(activeSynergies).flatMap(([side, ids]) => ids.map((synergyId) => ({ type: "battle.synergy-activated", day: 0, payload: { side, synergyId } })));
  const passiveEvents                = units.filter(({ active }) => active).map((unit) => ({ type: "battle.passive-activated", day: 0, payload: { side: unit.side, monsterId: unit.id, passiveId: unit.species.passiveId } }));
...
export function validActions(state             , actorId        , content                                                         )                          {
  ...
  const actions                 = enemies.map((target) => ({ kind: "basic", actorId, targetId: target.id }));
  ...
  for (const skillId of actor.monster.equippedSkillIds) {
    ...
    if (!skill || actor.energy < actionEnergyCost(actor, skill, content                 ) || (actor.cooldowns[skill.id] ?? 0) > 0) continue;
...
function targetsFor(state             , actor            , action              , skill                  )               {
...
export function applyBattleAction(state             , action              , content               , rng              , day = 0)              {
  ...
  for (const targetSnapshot of targets) {
    ...
    if (power > 0 && target.side !== acting.side) {
      ...
      let multiplier = typeMultiplier(attackType, target.species.types.filter(Boolean)              );
  ...
  const result               = !enemyAlive ? "player-victory" : !playerAlive ? "enemy-victory" : "ongoing";
...
export function chooseAiAction(state             , actorId        , content               )               {
  ...
  return actions.map((action) => {
    ...
    if (target && target.side !== actor.side) {
      score *= typeMultiplier(skill?.type ?? actor.species.types[0], target.species.types.filter(Boolean)              );
